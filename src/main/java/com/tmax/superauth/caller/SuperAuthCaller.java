package com.tmax.superauth.caller;
...
@Slf4j
public class SuperAuthCaller {
    static OkHttpClient client = new OkHttpClient();

    private static String setSuperAuthURL(String serviceName) {
        int superauthHttpPort = 8080;
        if (StringUtil.isNotEmpty(System.getenv("SUPERAUTH_HTTP_PORT"))) {
            superauthHttpPort = Integer.parseInt(System.getenv("SUPERAUTH_HTTP_PORT"));
        }
        return Constants.SUPERAUTH_URL + ":" + superauthHttpPort + "/" + serviceName;
    }

    public static String loginAsAdmin() throws IOException {
        log.info(" [SuperAuth] Login as Admin Service");

        Request request = null;

        //POST svc
        HttpUrl.Builder urlBuilder = HttpUrl.parse(setSuperAuthURL(Constants.SERVICE_NAME_LOGIN_AS_ADMIN)).newBuilder();
    ...
    public static JsonObject getUser(String userId, String token) throws IOException {
        log.info(" [SuperAuth] SuperAuth Get User Detail Service");

        Request request = null;

        //GET svc
        HttpUrl.Builder urlBuilder = HttpUrl.parse(setSuperAuthURL(Constants.SERVICE_NAME_USER_DETAIL) + userId).newBuilder();
        log.debug(" setSuperAuthURL(Constants.SERVICE_NAME_USER_DETAIL ) + userId" + setSuperAuthURL(Constants.SERVICE_NAME_USER_DETAIL) + userId);
    ...
    public static JsonArray getUserList(String token, int first, int max) throws IOException {
        log.info(" [SuperAuth] SuperAuth Get User List Service");

        Request request = null;

        //GET svc
        HttpUrl.Builder urlBuilder = HttpUrl.parse(setSuperAuthURL(Constants.SERVICE_NAME_USER_DETAIL)).newBuilder().addQueryParameter("first", Integer.toString(first));
    ...
    public static int getUserCount(String token) throws IOException {
        log.info(" [SuperAuth] SuperAuth Get User Count Service");

        Request request = null;

        //GET svc
        HttpUrl.Builder urlBuilder = HttpUrl.parse(setSuperAuthURL(Constants.SERVICE_NAME_USER_COUNT)).newBuilder();
    ...
    public static JsonObject deleteUser(String userId, String token) throws IOException {
        log.info(" [SuperAuth] SuperAuth User Delete Service");
    ...
    public static JsonObject getRealmInfo(String realmName, String token) throws IOException {
        log.info(" [SuperAuth] Realm Info Get Service");

        Request request = null;

        //GET svc
        HttpUrl.Builder urlBuilder = HttpUrl.parse(setSuperAuthURL(Constants.SERVICE_NAME_REALM_DETAIL) + realmName).newBuilder();
