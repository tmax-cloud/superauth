...
import com.tmax.superauth.caller.SuperAuthCaller;

import com.tmax.superauth.eventlistener.kafka.producer.KafkaProducer;
import com.tmax.superauth.rest.Util;
...
public class UserDeleteJob implements Job {
    ...
    public void execute(JobExecutionContext context) {
        ...
        // CronJob 선언부는 SuperauthEventListenerPropviderFactory PostInit()에 존재
        ...
        try{
            ...
            accessToken = SuperAuthCaller.loginAsAdmin();

            // Get User Count
            int userCount = SuperAuthCaller.getUserCount(accessToken);
            ...
            for (int i = 0; i < (userCount / USER_COUNT_UNIT) + 1; i++){
                ...
                JsonArray userListGet = SuperAuthCaller.getUserList(accessToken,first, USER_COUNT_UNIT);
                users.addAll(userListGet);
            }
        }catch( Exception e){
            log.error(" [UserDelete Job] SuperAuth Not Ready yet ");
            ...
            try {
                if ( userRepresentation.getAttributes() != null && userRepresentation.getAttributes().get("deletionDate") != null){
                    ...
                    if ( currentDate.after(deletionDate)){
                        ...
                        SuperAuthCaller.deleteUser(userRepresentation.getId(), accessToken);
