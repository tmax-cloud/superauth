package com.tmax.superauth.eventlistener.kafka.producer;
...
//                      important : session에는 이미 user가 지워져서 user 정보를 들고 올수 없음 그래서 http콜로 한다!
                        String accessToken = SuperAuthCaller.loginAsAdmin();
                        JsonObject user = SuperAuthCaller.getUser(adminEvent.getResourcePath().toString().substring(6), accessToken.replaceAll("\"", ""));
